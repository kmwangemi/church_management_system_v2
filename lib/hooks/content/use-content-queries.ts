...
const updateContent = async (
  ...
  const { data } = await apiClient.put(`/content?id=${id}`, payload);
  return data;
};
...
const incrementViewCount = async (
  ...
  const { data } = await apiClient.patch(`/content/${id}/view`);
  return data;
};
...
const incrementDownloadCount = async (
  ...
  const { data } = await apiClient.patch(`/content/${id}/download`);
  return data;
};
...
const bulkUpdateContentStatus = async (
  ...
  const { data } = await apiClient.patch('/content/bulk/status', {
    ids,
    status,
  });
  return data;
...
const bulkDeleteContent = async (
  ...
  const { data } = await apiClient.delete('/content/bulk', { data: { ids } });
  return data;
};
...
const fetchPopularContent = async (
  ...
  const { data } = await apiClient.get(
    `/content/popular?limit=${limit}&timeframe=${timeframe}`
  );
  return data;
};
