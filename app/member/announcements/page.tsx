...
import { MessageSquare, Search, Pin, Heart, Share, Bookmark, Clock, AlertCircle, Info, CheckCircle } from "lucide-react"
...
export default function MemberAnnouncements() {
  ...
  const filteredAnnouncements = announcements.filter((announcement) => {
    ...
    const matchesCategory =
      filterCategory === "all" || announcement.category.toLowerCase() === filterCategory.toLowerCase()
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Stay updated with the latest news and announcements from your church</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-2">
                    {announcement.pinned && <Pin className="h-4 w-4 text-yellow-600" />}
                    {getPriorityIcon(announcement.priority)}
                    <Badge variant="outline" className={getPriorityColor(announcement.priority)}>
                      {announcement.priority.charAt(0).toUpperCase() + announcement.priority.slice(1)}
                    </Badge>
                    <Badge variant="secondary">{announcement.category}</Badge>
                    {announcement.readStatus === "unread" && <Badge className="bg-blue-100 text-blue-800">New</Badge>}
                  </div>
