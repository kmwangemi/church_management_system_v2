...
// PUT /api/church/groups/[id]/goals/[goalId] - Update group goal progress
async function updateGroupGoalHandler(
  request: NextRequest,
  { params }: RouteParams
  ...
  const contextLogger = logger.createContextLogger(
    {
      requestId,
      endpoint: `/api/church/groups/${groupId}/goals/${goalId}`,
    },
    'api'
  );
  try {
    ...
    if (status && !Object.values(GroupGoalStatus).includes(status)) {
      return NextResponse.json(
        {
          error: `Invalid goal status. Must be one of: ${Object.values(GroupGoalStatus).join(', ')}`,
        },
        { status: 400 }
      );
    }
    ...
    // Find the goal
    const goalIndex = group.goals.findIndex(
    ...
    if (progress !== undefined && progress !== goal.progress) {
      ...
      if (progress === 100 && goal.status !== GroupGoalStatus.COMPLETED) {
        goal.status = GroupGoalStatus.COMPLETED;
        goal.completedAt = new Date();
        updatedFields.push('status (auto-updated)');
      } else if (progress > 0 && goal.status === GroupGoalStatus.PLANNED) {
    ...
    const updatedGoal = group.goals[goalIndex];
    contextLogger.info('Group goal updated successfully', {
      groupId,
      goalId,
      updatedFields,
      statusChange:
    ...
    contextLogger.error('Unexpected error in updateGroupGoalHandler', error);
...
// DELETE /api/church/groups/[groupId]/goals/[goalId] - Remove group goal
async function removeGroupGoalHandler(
  request: NextRequest,
  { params }: RouteParams
): Promise<NextResponse> {
  const { groupId, goalId } = await params;
  ...
  try {
    ...
    const removedGoal = group.goals[goalIndex];
    // Remove goal from array
    group.goals.splice(goalIndex, 1);
    await group.save();
    contextLogger.info('Group goal removed successfully', {
      groupId,
      goalId,
    ...
    contextLogger.error('Unexpected error in removeGroupGoalHandler', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
...
export const PUT = withApiLogger(updateGroupGoalHandler, {
  logRequests: true,
  logResponses: true,
  logErrors: true,
});

export const DELETE = withApiLogger(removeGroupGoalHandler, {
  logRequests: true,
  logResponses: true,
  logErrors: true,
});
