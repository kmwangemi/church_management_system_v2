...
export default function ChurchesPage() {
  ...
  console.log('organizations--->', JSON.stringify(churches));
  ...
  const filteredChurches = churches?.filter((church) => {
    const matchesSearch =
      ...
      church?.metadata?.denomination.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStatus =
      statusFilter === 'all' || church?.metadata.status === statusFilter;
    const matchesPlan = planFilter === 'all' || church?.metadata.plan === planFilter;
  ...
  const handleSaveChurch = async (updatedChurch: any) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="font-bold text-2xl">
              {churches?.reduce((sum, church) => sum + church?.metadata.members, 0)
                .toLocaleString()}
            </div>
            <p className="text-muted-foreground text-xs">
        ...
        <Card>
          ...
          <CardContent>
            <div className="font-bold text-2xl">
              {churches?.reduce((sum, church) => sum + church?.metadata.branches, 0)}
            </div>
            <p className="text-muted-foreground text-xs">
        ...
        <Card>
          ...
          <CardContent>
            <div className="font-bold text-2xl">
              $
              {churches?.reduce((sum, church) => sum + church?.metadata.revenue, 0)
                .toLocaleString()}
            </div>
            <p className="text-muted-foreground text-xs">
              ...
              <TableBody>
                {filteredChurches?.map((church) => (
                  <TableRow key={church.id}>
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <div className="text-muted-foreground text-sm">
                            {church?.metadata.denomination} • Est. {church?.metadata.established}
                          </div>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div>
                        <div className="font-medium">{church?.metadata.pastor}</div>
                        <div className="text-muted-foreground text-sm">
                          Lead Pastor
                        </div>
                      </div>
                    ...
                    <TableCell>
                      <div className="flex items-center">
                        ...
                        {church?.metadata.members.toLocaleString()}
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center">
                        <MapPin className="mr-1 h-4 w-4 text-muted-foreground" />
                        {church?.metadata.branches}
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center">
                        <DollarSign className="mr-1 h-4 w-4 text-muted-foreground" />
                        ${church?.metadata.revenue.toLocaleString()}
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center text-green-600">
                        <TrendingUp className="mr-1 h-4 w-4" />+{church?.metadata.growth}%
                      </div>
                    </TableCell>
                    <TableCell>{getStatusBadge(church?.metadata.status)}</TableCell>
                    <TableCell>{getPlanBadge(church?.metadata.plan)}</TableCell>
